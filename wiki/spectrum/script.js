import { getAmeliorates } from "../data.js";
...
function addImage(url, xNorm, yNorm, scale)
{
    ...
    {
        img.style.left = "calc(" + (xNorm * 100) + "% - " + (this.width * scale * (1/2)) + "px)";
...
function redrawMarkers(scale, zeropacity)
{
    ...
    {
        const daMonsterThing = monsterArray[i];
        const daMonster = daMonsterThing.monster;

        const img = placeImageAtHex(daMonster.dominantColor, daMonster.images.default, scale);
        if (zeropacity) img.style.opacity = "0";
    }
}

function resizeCanvas(zeropacity = true)
{
    ...
    redrawMarkers(scale, zeropacity);
