import { getClasses, getMonsters, getRarities, getElements, stringToElementSigil } from "../monsters.js";
...
export async function makeNaturalTable(daIsland, daElements = new Set())
{
    ...
    const coolMonsters = monsters.filter((mon) => ((mon.rarity == RARITY.EPIC && mon.class == MCLASS.NATURAL)
                                                ...
                                                && mon.rarity != RARITY.RARE).reverse();
            ...
            if (x + y > colMonsters.length - 2)
            {
                ...
                if (rowMon == colMon)
                {
                    ...
                    continue;
                        ...
                        || (rowMon.class == MCLASS.FIRE || rowMon.class == MCLASS.FIRE)
                        || (rowMon.class == MCLASS.MAGICAL || rowMon.class == MCLASS.MAGICAL)
                    )
                {
                    const rareRowMon = rowMonsters.find((mon) => areSetsEqual(rowMon.elements, mon.elements));
                ...
                for (const mon of breedMonsters)
                {
                    ...
                    else if (mon.class == MCLASS.ETHEREAL)
                    {
                    ...
                    const rareMon = monsters.find((m) => m.elementString == mon.elementString && m.rarity == RARITY.RARE);
                    if (mon.rarity == RARITY.COMMON && rareMon != null)
                    {
